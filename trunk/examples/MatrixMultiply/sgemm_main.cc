...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    Matrix<float> a = Matrix<float>::Rand(m, k);
    Matrix<float> b = Matrix<float>::Rand(k, n);
    Matrix<float> c = Matrix<float>::Rand(m, n);

    cout << "Converting matrices..." << endl;

    BlockedMatrix<float> aBlocked(a);
    BlockedMatrix<float> bBlocked(b);
    BlockedMatrix<float> cBlocked(c);
